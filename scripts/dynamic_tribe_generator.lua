...
local function weighted_pick_random_no_replace(t)
    ...
    if index_to_remove > 0 then
        -- Swap with last element and pop - O(1) instead of O(n) table.remove
        local selected = t[index_to_remove]
        local last_idx = #t
        if index_to_remove ~= last_idx then
            t[index_to_remove] = t[last_idx]
        end
        t[last_idx] = nil
        return selected
    end
...
-- Pre-computed set for O(1) rating lookups
local VALID_RATINGS_SET = {
    SPNT = true, DVNT = true, HSTL = true,
    VSSL = true, AVTR = true, NCUR = true
}

-- OPTIMIZED: Uses direct indexing instead of table.insert
...
-- OPTIMIZED: Fast table merge using direct indexing
local function table_merge_fast(t1, t2)
    local t1_len = #t1
    local t2_len = #t2
    for i = 1, t2_len do
        t1[t1_len + i] = t2[i]
...
-- ====================================================================
-- PRE-PARSED UNIVERSAL RAWS (parsed once at module load, not per-entity)
-- ====================================================================
local UNIVERSAL_ENTITY_RAWS_PARSED = {}
for line in UNIVERSAL_ENTITY_RAWS:gmatch("[^\r\n]+") do
    UNIVERSAL_ENTITY_RAWS_PARSED[#UNIVERSAL_ENTITY_RAWS_PARSED + 1] = line
end
...
local function parse_creature_token(token)
    ...
    for i, part in ipairs(parts) do
        -- Use set lookup instead of string comparison chain (O(1) vs O(k))
        if VALID_RATINGS_SET[part] then
            rating_index = i
            rating = part
            break
        end
                ...
                if parsed and parsed.archetype and civ_configuration[parsed.archetype] then
                    ...
                    if effective_sub and config.subs[effective_sub] then
                        ...
                        if sub_config.allowed_ratings then
                            -- Use cached set if available, otherwise check array
                            if sub_config._allowed_ratings_set then
                                rating_ok = sub_config._allowed_ratings_set[parsed.rating] or false
                            else
                                -- Build set cache on first access
                                sub_config._allowed_ratings_set = {}
                                for _, r in ipairs(sub_config.allowed_ratings) do
                                    sub_config._allowed_ratings_set[r] = true
                                end
                                rating_ok = sub_config._allowed_ratings_set[parsed.rating] or false
                            end
                        end
...
local function generate_civilizations_hook()
    ...
    while #eligible_tribe_creatures > 0 and civ_counter <= max_tribe_civs do
        ...
        if candidate then
            ...
            -- OPTIMIZED: Use pre-parsed table instead of re-parsing string each time
            table_merge_fast(civ_tbl, UNIVERSAL_ENTITY_RAWS_PARSED)
    ...
    if #eligible_tribe_creatures > 0 then
        log("TRIBE_GEN_LOG: Cap reached (" .. max_tribe_civs .. "). Skipping remaining tribe civs.")
    end
end
...
M.register_all_generated_civilizations = function()
    if raws and raws.register_entities and #all_generated_tribe_civ_raws > 0 then
        -- OPTIMIZED: Batch all entity lines into single registration call
        local all_lines = {}
        local idx = 0
        ...
        raws.register_entities(all_lines)
        log("TRIBE_GEN_LOG: Batch registered " .. #all_generated_tribe_civ_raws .. " tribe civilizations (" .. idx .. " lines)")
    end
end
