 local M = {}
    ...
    local function generate_npc_tiers(config, sorted_ability_keys)
        ...
        for i = 0, config.MAX_POWER_LEVEL do table.insert(lines, "            [SYN_AFFECTED_CLASS:INNATE_" .. config.INNATE_CLASS .. "_" .. i .. "]") end
                ...
                for sev = 1, config.MAX_POWER_LEVEL do
                    for _, target_type_key in ipairs(ability.target_types or {}) do
                        ...
                        if ability_tier == tier_to_grant then
                            ...
                            if not (ability.role == "DEFENSE" and target_type_key ~= "SELF") then
                                local strict_name = string.format("%s_SEV_%d_%s", ability.token_prefix, sev, target_type_key)
                                local unique_class = "HAS_" .. strict_name

                                local prob = 30
                                -- Mandatory guard rule: POSPROB is excluded from 100% prob because it is non-mandatory.
                                if ability.role == "DEFENSE" and target_type_key == "SELF" and ability.token_prefix ~= "POSPROB" then prob = 100 end

                                table.insert(lines, "        [SYNDROME]")
                                table.insert(lines, "            [SYN_CLASS:" .. unique_class .. "]\n            [SYN_IMMUNE_CLASS:" .. unique_class .. "]")
                                table.insert(lines, "            [SYN_AFFECTED_CLASS:INNATE_" .. config.INNATE_CLASS .. "_" .. firewall_lvl .. "]")
                                ...
                                table.insert(lines, "                [CDI:WAIT_PERIOD:" .. (ability_tier * 100) .. "]")
                                table.insert(lines, "                [CDI:USAGE_HINT:" .. (ability.role == "DEFENSE" and "DEFEND" or "ATTACK") .. "]")

                                local role_text = config.FLAVOR_TEXT[ability.role]
                                local target_order = target_type_key  -- CHANGED: Use target type string instead of numeric order
                                if role_text and role_text[sev] and role_text[sev][target_order] then
                                    local entry = role_text[sev][target_order]
                                    table.insert(lines, "                [CDI:VERB:" .. entry.VERB_1ST .. ":" .. entry.VERB_3RD .. ":NA]")
                                else
                                    table.insert(lines, "                [CDI:VERB:compile a program:compiles a program:NA]")
                                end
