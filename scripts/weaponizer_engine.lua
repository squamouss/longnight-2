...
-- Ensure weapon has cached sets for O(1) eligibility checks
local function ensure_weapon_sets_cached(weapon_data)
    if weapon_data._sets_cached then return end
    if weapon_data.eligible_archetypes then
        weapon_data._eligible_archetypes_set = array_to_set(weapon_data.eligible_archetypes)
    end
    if weapon_data.eligible_size_tags then
        weapon_data._eligible_size_tags_set = array_to_set(weapon_data.eligible_size_tags)
    end
    if weapon_data.eligible_rating_tags then
        weapon_data._eligible_rating_tags_set = array_to_set(weapon_data.eligible_rating_tags)
    end
    if weapon_data.eligible_sub_archetypes then
        weapon_data._eligible_sub_archetypes_set = array_to_set(weapon_data.eligible_sub_archetypes)
    end
    weapon_data._sets_cached = true
end
...
local function is_eligible(creature_data, weapon_data, debug_mode)
    -- Ensure O(1) lookup sets are cached
    ensure_weapon_sets_cached(weapon_data)

    if weapon_data.eligible_archetypes then
        local arch_match = false
        local arch_set = weapon_data._eligible_archetypes_set
        if creature_data.archetypes and arch_set then
            for _, arch in ipairs(creature_data.archetypes) do
                if arch_set[arch] then
                    arch_match = true
                    break
                end
            end
    ...
    if weapon_data.eligible_size_tags then
        local size_set = weapon_data._eligible_size_tags_set
        if not (size_set and size_set[creature_data.size_tag]) then
            return false
        end
    end
    if weapon_data.eligible_rating_tags then
        local rating_match = false
        local rating_set = weapon_data._eligible_rating_tags_set
        if creature_data.rating_tags and rating_set then
            for _, tag in ipairs(creature_data.rating_tags) do
                if rating_set[tag] then
                    rating_match = true
                    break
                end
            end
    ...
    if weapon_data.eligible_sub_archetypes then
        ...
        local sub_set = weapon_data._eligible_sub_archetypes_set
        if creature_data.sub_archetypes and sub_set then
...
-- Lazily convert conflict_table arrays to sets for O(1) lookup
local conflict_table_sets_cache = {}
local function get_conflict_set(conflict_table, theme)
    if not conflict_table or not conflict_table[theme] then return nil end
    -- Use the conflict_table as part of the cache key identity
    if not conflict_table_sets_cache[conflict_table] then
        conflict_table_sets_cache[conflict_table] = {}
    end
    local cache = conflict_table_sets_cache[conflict_table]
    if not cache[theme] then
        cache[theme] = array_to_set(conflict_table[theme])
    end
    return cache[theme]
end

-- Check conflict between two themes (optimized with cached sets)
local function themes_are_conflicting(t1, t2, conflict_table)
    ...
    -- Check T1 vs T2 using cached set
    local set1 = get_conflict_set(conflict_table, u1)
    if set1 and set1[u2] then return true end
    -- Check T2 vs T1 using cached set
    local set2 = get_conflict_set(conflict_table, u2)
    if set2 and set2[u1] then return true end
...
function M.apply_weapons(raw_lines, weapon_config, forced_theme, creature_metadata)
    ...
    for i, loadout in ipairs(potential_loadouts) do
        ...
        for _, block in ipairs(original_caste_blocks) do
            ...
            -- === NEW FEATURE: Inject Universal Theme Data ===
            if weapon_config.UNIVERSAL_THEME_DATA and weapon_config.UNIVERSAL_THEME_DATA[selected_theme] then
                local extra_raws = weapon_config.UNIVERSAL_THEME_DATA[selected_theme]
                table.insert(final_lines, extra_raws)
            end
            -- ================================================
