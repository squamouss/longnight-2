...
-- [[ OPTIMIZATION: Pre-compile text to avoid Regex thrashing in loops ]] --
local function precompile_caste_block(lines)
    local template = {}
    for _, line in ipairs(lines) do
        -- Skip the CASTE definition line itself, as we generate new ones manually
        if not line:match("^%s*%[CASTE:") then
            local pop_str = line:match("%[POP_RATIO:(%d+)%]")
            if pop_str then
                table.insert(template, { type = "POP", value = tonumber(pop_str) })
            elseif line:match("^%s*%[DESCRIPTION:") then
                -- Extract description content once
                local desc = line:match("^%s*%[DESCRIPTION:(.+)%]"):gsub("%]$", "")
                table.insert(template, { type = "DESC", value = desc })
            elseif line:match("^%s*%[CASTE_NAME:") then
                table.insert(template, { type = "NAME", value = line })
            else
                table.insert(template, { type = "STATIC", value = line })
            end
        end
    end
    return template
...
function M.apply_weapons(raw_lines, weapon_config, forced_theme, creature_metadata)
    ...
    local function recursive_combine(group_idx, current_loadout)
        ...
        if #potential_loadouts >= 100 then return end 
    ...
    -- Convert raw lines into templates to avoid regex in the hot loop
    for _, block in ipairs(original_caste_blocks) do
        block.template = precompile_caste_block(block.lines)
    end
    -- [[ END OPTIMIZATION ]] --
    ...
    for i, loadout in ipairs(potential_loadouts) do
        ...
        for _, block in ipairs(original_caste_blocks) do
            ...
            for _, item in ipairs(block.template) do
                if item.type == "POP" then
                    -- Constant Time Math
                    local new_ratio = math.max(1, math.floor(item.value * combined_pop_mult))
                    table.insert(final_lines, "[POP_RATIO:" .. new_ratio .. "]")
                elseif item.type == "DESC" then
                    -- Direct function call, no searching for the line
                    local new_desc_line = build_modified_description(item.value, loadout)
                    table.insert(final_lines, new_desc_line)
                elseif item.type == "NAME" then
                    -- Direct injection
                    caste_name_found = true
                    local new_name_line = inject_suffix_into_name_tag(item.value, loadout_suffix, true)
                    table.insert(final_lines, new_name_line)
                elseif item.type == "STATIC" then
                    -- Zero processing
                    table.insert(final_lines, item.value)
                end
            end
            -- [[ END OPTIMIZATION ]] --
