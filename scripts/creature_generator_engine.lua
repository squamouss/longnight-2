    ...
    if status_wep and status_cfg and weaponizer and weapon_config then
        ...
        for _, obj in ipairs(generated_data) do
            if obj.id and obj.components then
                ...
                -- [[ FIX: Manually gather sub-archetypes from components ]]
                local collected_sub_archetypes = {}
                if obj.components.sub_archetypes then
                    for _, sa in ipairs(obj.components.sub_archetypes) do 
                        table.insert(collected_sub_archetypes, sa) 
                    end
                end
                
                -- Iterate through component definitions to find their sub-archetypes
                if config.COMPONENT_TABLES then
                    for _, conf in ipairs(config.COMPONENT_TABLES) do
                        local comp = obj.components[conf.name]
                        if comp and comp.sub_archetype then
                            for _, sa in ipairs(comp.sub_archetype) do
                                -- Avoid duplicates
                                local exists = false
                                for _, existing in ipairs(collected_sub_archetypes) do
                                    if existing == sa then exists = true; break end
                                end
                                if not exists then
                                    table.insert(collected_sub_archetypes, sa)
                                end
                ...
                local adapter_obj = {
                    ...
                    pop_ratio = 50,
                    sub_archetypes = collected_sub_archetypes
                }
        ...
        for i, obj in ipairs(generated_data) do
             local meta = weaponizer_input[obj.id]
             local new_raws = weaponizer.apply_weapons(obj.creature, weapon_config, nil, meta)
