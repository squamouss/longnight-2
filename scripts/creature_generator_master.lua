...
-- Using pcall for optional modules to prevent crashes if a rule file is missing
    ...
    for _, component in ipairs(component_list) do
        ...
        if archetype_ok then
            ...
            if rating_ok then
                ...
                -- Only check strict size restrictions if we are currently selecting for a specific size (size_tags > 0).
                -- If size_tags is empty (e.g. during pre-validation), skip this check to allow the component to pass validity.
...
function M.gather_caste_toks(config, components, caste_type)
    ...
    for _, comp in ipairs(all_components_for_tokens) do
        if comp then
            ...
            if tok_block then
                M.split_to_lines(all_toks, tok_block)
            end
        end
    end
    return all_toks
    ...
    for _, comp in ipairs(all_components_for_tokens) do
        if comp and comp.tok then
            ...
            if not has_caste_override then
                M.split_to_lines(all_toks, comp.tok)
            end
        end
    end
    return all_toks
...
function M.safe_get_gait_tokens(config, components)
    ...
    -- Attempt to find the correct function in the gait calculator
    local func = M.gait.generate_gait_lines or M.gait.get_gait_tokens or M.gait.calculate_gaits
    if func then
