...
-- ====================================================================
-- Hierarchy: Archetype -> Sub-Archetype -> Rating -> Size -> Entity Config
-- WEIGHTS are now handled here (e.g. weight = 20).

local civ_configuration = {
    
    -- === CASTE RACES (Use Fallback) === --
    LOCASTE = {
...
local function parse_creature_token(token)
    -- Token format example: BIOMACHINE_ID_MDM_SPNT_1234_DEVIL
    -- Fallback example: LOCASTE_ID_MDM_SPNT_1234_UNKNOWN
    ...
    -- Scan for known Ratings to find our place in the token string
    ...
    if rating_index > 2 then
        size = parts[rating_index - 1] -- Size is immediately before Rating
        
        -- Sub-archetype is usually after the numeric code (Rating + 2)
        -- e.g. ..._SPNT_1234_DEVIL
        ...
        if creature_info.creature and #creature_info.creature > 0 then
            ...
            if token then
                ...
                if parsed and parsed.archetype and civ_configuration[parsed.archetype] then
                    ...
                    -- Determine Effective Sub-Archetype
                    local effective_sub = parsed.sub_archetype
                    
                    -- [[ FIX: Handle "UNKNOWN" or invalid sub-archetypes by falling back ]]
...
local function generate_civilizations_hook()
    ...
    for i = 1, expected_civ_amt do
        -- Pick from the weighted list and remove the chosen one to avoid duplicate creature-civs
        ...
        table.insert(_G.generated_civilization_data, {
