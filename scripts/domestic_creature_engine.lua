...
function generate_domestic_creatures_for_civs(generated_civs, domestic_rules)
    ...
    -- [[ FIX: Removed large object table for batch processing ]] --
                ...
                while not generated_successfully and max_attempts_per_creature > 0 do
                    ...
                    if selection_successful and components then
                        ...
                        if not global_fingerprints[fingerprint] then
                            ...
                            -- [[ START OF FIX: Inline Weaponizer Integration ]] --
                            -- Paste this immediately after generating your 'creature_raws' but BEFORE adding to the master list.
                            
                            if weaponizer_module and status_wep and weapon_configs then
                                -- Construct metadata using the variables available in your current loop scope
                                -- CHECK THESE VARIABLES: Ensure 'selected_rating', 'size_tag', etc. match your variable names
                                local metadata = {
                                    rating_tags = components.rating.tags, -- Using actual tags from component
                                    archetypes = { config.ARCHETYPE_CLASS or "DOMESTIC" },
                                    size_tag = (components.size_variant.size_tags and components.size_variant.size_tags[1]) or "MEDIUM",
                                    sub_archetypes = { effective_sub_archetype } 
                                }
                                
                                -- log_func("Applying weapons to " .. creature_id) -- Optional logging

                                -- Direct call to the core weaponizer function
                                local weapon_raws = weaponizer_module.apply_weapons(
                                    creature_data.creature,    -- The table of raw strings you just built
                                    weapon_configs,   -- The loaded config
                                    forced_theme, -- Theme if present
                                    metadata          -- The metadata wrapper
                                )

                                -- Merge results immediately
                                if weapon_raws and #weapon_raws > 0 then
                                    for _, line in ipairs(weapon_raws) do
                                        table.insert(creature_data.creature, line)
                                    end
                                    -- Add the marker class so we know it was processed
                                    table.insert(creature_data.creature, "[CREATURE_CLASS:WEAPONIZED_DOMESTIC]")
                                end
                            end
                            -- [[ END OF FIX ]] --
                            ...
                            -- [[ FIX: Removed large object table insert ]] --
    ...
    if _G.raws and _G.raws.register_creatures and #all_domestic_creature_raws > 0 then
        -- [[ FIX: Removed legacy batch weaponizer logic here ]] --
