...
function generate_domestic_creatures_for_civs(generated_civs, domestic_rules)
    ...
    -- [[ NEW: List to store object data for weaponization ]]
    ...
    for _, civ_info in ipairs(generated_civs) do
        ...
        -- [[ NEW LOGIC: Filter rules by required sub-archetype ]]
        local valid_rules_for_this_civ = {}
        for _, rule in ipairs(potential_config_rules) do
            local is_valid = true
            
            -- If the rule demands a specific sub-archetype, check if the civ matches it
        ...
        if #valid_rules_for_this_civ == 0 then
            -- Fallback: If filtering removed all options (rare), revert to the full list to prevent errors
        ...
        -- [[ LOGIC START: Determine Effective Sub-Archetype for Ratings ]]
        -- Calculate the effective sub-archetype (e.g. HUMAN -> DEVIL) to choose the correct rating table
        ...
        for rating_tag, override in pairs(rating_overrides) do
            ...
            -- ========================================================================================
            -- MODIFIED: Diminishing Returns Logic for Count Calculation
            -- ========================================================================================
            local num_to_gen = 0
            if override.min and override.max then
                -- Start with the minimum guaranteed
                num_to_gen = override.min
                
                -- Calculate potential extra creatures
                ...
                if potential_extras > 0 then
                    ...
                    for k = 1, potential_extras do
                        -- Generate a roll from 0.0 to 1.0
                        local roll = 0
                        if _G.trandom then
                        ...
                        if roll <= current_threshold then
                            ...
                            -- Decay the chance for the next one
                            current_threshold = current_threshold * decay_factor
                        else
                            -- Failed the roll for an extra creature, stop adding more (diminishing returns "streak" ends)
                            break
                        end
                    end
                end
                ...
                while not generated_successfully and max_attempts_per_creature > 0 do
                    ...
                    if selection_successful and components then
                        ...
                        if not global_fingerprints[fingerprint] then
                            ...
                            -- [[ NEW: CAPTURE DATA FOR WEAPONIZER ]]
                            -- We construct a data object matching the expected structure
                            local domestic_obj = {
                                id = creature_data.id,
                                raw_lines = creature_data.creature, -- [[ IMPORTANT: PASS RAW LINES ]]
                                ...
                                pop_ratio = 50 -- default for generated creatures
                            }
    ...
    if _G.raws and _G.raws.register_creatures and #all_domestic_creature_raws > 0 then
        -- [[ NEW: WEAPONIZER INTEGRATION EXECUTION ]]
        if weaponizer_module and weapon_configs and #all_domestic_creature_objects > 0 then
            ...
            -- Call the weaponizer's public API
            ...
            if weapon_raws and #weapon_raws > 0 then
                ...
                -- Append weapon raws as a new entry in the main list
...
-- The main function to be called by init.lua
